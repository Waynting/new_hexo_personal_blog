...
**結果：**
- 圖片會自動上傳到 R2：`blog-post/2025/11/my-article/photo1.webp`
- 文章中的路徑會被替換為：`https://img.waynspace.com/2025/11/my-article/photo1.webp`
- 本地圖片會被自動刪除
...
**結果：**
- 文章中的 `images/photo.jpg` 會被替換為：`https://img.waynspace.com/2025/11/my-article/photo.webp`
...
categories:
  - 技術筆記
coverImage: "https://img.waynspace.com/2025/11/my-new-article/cover.webp"
---

這是文章內容。

![截圖1](https://img.waynspace.com/2025/11/my-new-article/screenshot1.webp)
![截圖2](https://img.waynspace.com/2025/11/my-new-article/screenshot2.webp)
```
...
**支援的路徑格式：**
- 相對路徑：`./image.jpg`（推薦）
- 絕對路徑：`/Users/waynliu/Pictures/image.jpg`
- file:// 協議：`file:///Users/waynliu/Pictures/image.jpg`
...
**注意**：需要先配置 rclone。此腳本適用於舊的目錄結構，推薦使用 `process-local-images.mjs`。
...
A: 推薦使用 `npm run images:process`，它會自動處理所有圖片。如果需要手動上傳：
...
# 然後在文章中使用完整 URL
![圖片](https://img.waynspace.com/2025/11/my-article/photo.jpg)
```
...
- ✅ 或使用 `npm run images:delete-spaces` 刪除 R2 上包含空格的文件
...
2. **將圖片放在文章同目錄下**：方便管理，處理後自動刪除
3. **使用相對路徑**：`./image.jpg` 比絕對路徑更方便
4. **使用 WebP 格式**：腳本會自動轉換，減小檔案大小
5. **定期驗證**：使用 `npm run images:verify` 檢查圖片是否正確
6. **使用快取**：腳本會自動快取已處理的圖片，避免重複上傳
7. **處理完成後刪除本地圖片**：保持目錄整潔，只提交 Markdown 文件到 Git

---

## 📝 完整工作流程

1. **撰寫文章** → `content/YYYY/MM/my-article.md`
2. **準備圖片** → 放在文章同目錄下
3. **使用相對路徑** → `./image.jpg`
4. **執行處理** → `npm run images:process`
5. **提交到 Git** → 只提交 `.md` 文件
